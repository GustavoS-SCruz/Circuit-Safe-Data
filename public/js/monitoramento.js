...
document.addEventListener("DOMContentLoaded", function () {
	...
	(async () => {
		const networkData = await fetch("https://demo-live-data.highcharts.com/aapl-c.json")
        .then(response => response.json());

		const formatTimestamps = (dataArray) => {
			return dataArray.map(item => {
				const timestamp = item[0];
				const reading = item[1] / 2;

	            return [timestamp, reading];
			});
		};
    	const formattedNetworkData = formatTimestamps(networkData);
